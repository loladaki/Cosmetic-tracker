...
MIN_SEMANAS = 4  # semanas de historico minimo antes de disparar alertas
...
def get_fallback(cursor, produto):
    cursor.execute("""
...
# ANÁLISE DE ALERTAS (comparação semanal — folheto a folheto)

def analisar_alertas(cursor, hoje, dados):
    # Semana do Continente: terca a segunda
    ...
    for item in dados:
        ...
        # Historico semanal (excluindo semana atual)
        cursor.execute("""
            SELECT
        ...
        """, (produto, inicio_semana_atual))
        semanas_hist = cursor.fetchall()

        n_semanas = len(semanas_hist)
        if n_semanas < MIN_SEMANAS:
            print(f"  {produto[:40]}: {n_semanas}/{MIN_SEMANAS} semanas -- a acumular historico")
            continue
        ...
        # ALERTA 1: desconto acima da media historica semanal
        ...
        # ALERTA 2: preco minimo historico
...
def enviar_email(hoje, alertas_media, alertas_minimo):
    ...
    for a in sorted(alertas_media, key=lambda x: x["diferenca"], reverse=True):
        ...
        <tr>
            ...
            <td style="color:#888">{a['n_semanas']} semanas</td>
        </tr>"""
    ...
    if alertas_media:
        ...
        <p style="font-size:13px;color:#555">
            Estes produtos têm um desconto superior à média das semanas anteriores.
        </p>
        <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-size:13px">
            <tr style="background:#f2f2f2">
                <th>Produto</th><th>Preco hoje</th><th>PVPR</th>
    ...
    if alertas_minimo:
        ...
        <p style="font-size:13px;color:#555">
            Estes produtos estão no preco mais baixo de sempre.
        </p>
        <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-size:13px">
            <tr style="background:#f2f2f2">
                <th>Produto</th><th>Preco hoje</th><th>PVPR</th>
    ...
    <html><body style="font-family:Arial,sans-serif;padding:20px;max-width:900px">
        ...
        <p>
            <b>{len(alertas_media)}</b> produto(s) com desconto acima da media semanal e
        ...
        <p style="font-size:11px;color:#aaa">
            Alerta automatico · Cosmetica Continente · comparacao semanal ·
    ...
    try:
        ...
        print(f"Email enviado: {len(alertas_media)} alertas media + {len(alertas_minimo)} minimos historicos.")
...
for produto, url in produtos.items():
    ...
    if preco is None:
        preco, pvpr, desconto_percent, desconto_euros = get_fallback(cursor, produto)
        if preco is not None:
            print(f"Fallback usado para '{produto[:40]}' -- preco anterior: {preco:.2f} EUR")
        else:
            print(f"Atencao: sem preco e sem fallback para '{produto[:40]}' -- ignorado")
            continue
...
for item in dados:
    cursor.execute("""
        INSERT INTO cosmetica_precos
            (data, produto, preco, pvpr, desconto_percent, desconto_euros)
        VALUES (%s, %s, %s, %s, %s, %s)
